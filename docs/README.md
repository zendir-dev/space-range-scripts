...
Reference and developer documentation for **Space Range**, a real-time, multi-team spacecraft operations simulator. These docs describe the public Message Queuing Telemetry Transport (MQTT) application programming interface (API), message formats, scenario configuration, and included tools such as the **Operator UI** and Python scripting framework. Use them to build clients, drive scenarios, and integrate Space Range with external systems.
...
- All times are in **simulation seconds** unless explicitly labeled as UTC or wall-clock.
