...
A **command** is a JSON object that a client publishes on a team's `Uplink` topic to drive a single spacecraft. Commands include immediate actions ("take a picture now"), time-tagged future actions ("fire the reaction control system (RCS) at T+1200 s"), and metadata operations ("remove a queued command").
...
A few non-obvious behaviors are worth knowing:
...
| You need to pre-stage encryption rotation | Schedule the `encryption` command. The spacecraft will be unreachable during the reboot interval. |
