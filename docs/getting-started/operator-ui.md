...
> [!WARNING]
> Treat these URLs like passwords. They include the team password in clear text. Do not paste them into shared chat channels or commit them to public repositories.
...
| **Control** | Forms for every spacecraft command (guidance, thrust, camera, jammer, power bus, RPO, docking, …). The fastest way to issue a single command. **Power**, **Guidance**, and **Camera** panels sync across operators via Configuration Report (APID 102). |
