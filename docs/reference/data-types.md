...
This page defines every type, unit, and convention used across the Space Range API, including angle and frequency units.
...
| **Vector2** | JSON array of 2 numbers `[x, y]` | two `float64` BE | Studio always serializes as 64-bit on the wire even though JSON shows them as plain numbers. |
...
Every entity in Space Range has at least one identifier. Each identifier has a specific shape and API scope.
...
| Type | Unit | Origin | Where it appears |
| --- | --- | --- | --- |
...
| **Real time (client)** | varies | client machine | `time.time()` and similar values; not interchangeable with Session `time`. |

### Conversion

The session topic is the bridge. Every Session message provides:
...
| `altitude` | meters | depends | Altitude above the surface in meters unless stated otherwise. |
...
Component `position` is in **meters** in the spacecraft body frame; `rotation` is in **degrees** as Euler angles (Yaw, Pitch, Roll). These follow Unreal Engine conventions internally but are exposed in standard SI units at the API surface.
...
| Value | Meaning |
| --- | --- |
| `idle` | No active pointing. Default for unrecognized modes. |
| `sun` | Track the Sun. |
| `nadir` | Track the planet center below. |
...
**The most common integration error** is sending lowercase keys to the spacecraft uplink, which Studio rejects. Check the envelope casing against [Spacecraft commands → Envelope](../api-reference/spacecraft-commands.md#envelope) before further debugging.
...
A missing optional field is **never** an error, and an unrecognized key is silently ignored. There is no "extra keys" warning. A misspelled key therefore causes Studio to execute the command without that field.
...
Inside `Commands` arrays in telemetry, the `Args` field has its `password` (and any future credential keys) **redacted** before transmission. The result has this form:
