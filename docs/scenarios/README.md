...
Example scenarios to copy from: `Orbital Intel`, `Maritime_Surveillance`, `Unresponsive_Satellite`, `Telemetry_Drop`, `Payload_Misalignment`, `Null_Data`, `Command_Rejection`, `Docking_Procedure`, `Cyber Defender`, `Tutorial`. Power-bus harness: `Testing/test_power_scenario`. Fuel-bus harness: `Testing/test_fuel_scenario`.
