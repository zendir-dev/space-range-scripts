...
| `integrator` | `string` | `"Euler"` | Numerical integrator. Accepts `"Euler"` or `"RK4"` (case-insensitive). Use `RK4` if a scenario depends on multi-orbit propagation accuracy (rendezvous, formation, long-duration maneuvers); `Euler` is fine otherwise. |
