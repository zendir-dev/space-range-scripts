...
- Choose distinct `color` values so objects can be referred to by color in imagery or inspection exercises.
- Space objects are **trackable by default**, so they appear in the **Operator UI** as relative-pointing targets. Because they have no components, `Aim Component` resolves to `None` (pointing at the object's center). Set `trackable: false` to hide one from targeting.
