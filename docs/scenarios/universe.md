...
| `magnetosphere` | `bool` | `false` | If `true`, Earth's magnetic field is modeled. Affects `Magnetometer` telemetry and any magnetorquer-style components. Set `false` to disable magnetic-field effects entirely. |
| `gps` | `bool` | `false` | If `true`, the `GPS Sensor` returns valid fixes. Set `false` to model a globally GPS-denied scenario (every spacecraft's GPS sensor returns no fix). For partial / regional GPS denial use a `GPS` event instead: see [events.md#gps-events](events.md#gps-events). |
...
| `ambient_light` | `number` `0.0–1.0` | `0.25` | Scene ambient light. Higher values brighten dark sides for visualization; **does not** affect physics or solar-panel power generation. |
...
- Set `gps` and `magnetosphere` explicitly. Both are `false` when their keys (or the entire `universe` block) are omitted.
...
- **Magnetic-field-aware exercise**: set `magnetosphere: true`. Most scenarios that use magnetic sensors enable it explicitly.
