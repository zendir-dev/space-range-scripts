...
| 2b — Rogue replay & jam | 00:30 – 00:50 | `9 000 – 15 000 s` | Random replay bursts; **light pulsed** uplink jam on Blue Bravo; **broadcast** downlink jam over both AOI overhead passes. |
...
1. **Open** `cyber_defender.json` in Studio (or load via the admin tooling). Verify `simulation.speed = 5.0`, `universe.gps = true`, and that the seven ground stations are present.
...
4. **Watch for these log markers** in the rogue's terminal — they confirm each phase started:

   - `capture: started — 2 blue team(s), dwell=750s, quota=2/team, window=[6000, 9000]s`
   - `capture: stored intercept for 'Blue Alpha' (1/2, rx=612 MHz, t≈6024s)` — repeats per intercept.
   ...
   - `replay: t=10324s burst 1/8 → 'Blue Bravo' @ 745 MHz (sha1=…)` — repeats per burst.
   - `jamming: scheduled 15 pulse(s) for 'Uplink Pulse Jam (Blue Bravo)' …`
...
- [ ] At `t=1 800 s`, the GPS spoof event fires — operator's GPS lat/lon snaps to the antipode while the defender is over Hormuz.
...
| --- | --- | --- |
| `T_AOI_1` | `8 700 s` | Find the actual sim-time of the first AOI overhead `(26.5° N, 56° E)` from the GPS trace; patch in. |
| `T_AOI_2` | `13 600 s` | Same for the second AOI overhead (one orbital period later). |
...
Add the extra entries to `teams[]` in the JSON (use the *Orbital Sentinel* roster as a template — the colours and frequencies are spread far enough apart to keep the EM-sensor view legible). No Python changes are required: `cyber_defender.py` reads `scenario.enemy_teams` dynamically, and the per-team capture quota auto-bumps from 2 → 3 once `len(enemy_teams) > 2`.
